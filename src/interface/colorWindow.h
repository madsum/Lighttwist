...
extern int do_color(int i);
extern int do_color_reset(int i);

#ifdef __cplusplus
}
#endif

class colorWindow : public Fl_Double_Window {
private:
  int LINEPOSY;
...
public:
  ...
  colorWindow(int x,int y,int w, int h,const char *l=0) : Fl_Double_Window(x,y,w,h,l){
    createMenuWindow();
    loadDefaultValues();
    set_modal();
    end();
